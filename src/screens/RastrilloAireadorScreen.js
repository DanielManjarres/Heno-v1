...
const RastrilloAireadorScreen = ({ navigation }) => {
  const { user } = useUser();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log('Obteniendo último registro de actividad para ID_actividad: 2 y ID_trabajador:', userId);
        const activity = await getLastActivity(2, userId, 'En curso');
  ...
  const handleStartActivity = async () => {
    ...
    try {
      ...
      const result = await startActivity(2, userId, location.ID_ubicacion, currentDate, currentTime);
      console.log('Resultado de iniciar actividad:', result);

      Alert.alert('Éxito', 'Actividad iniciada correctamente.');

      console.log('Obteniendo última actividad después de iniciar para ID_actividad: 2 y ID_trabajador:', userId);
      const activity = await getLastActivity(2, userId, 'En curso');
    ...
    return (
      ...
      >
        ...
        <View style={styles.container}>
          ...
          <Text style={styles.subtitle}>RASTRILLO AIREADOR</Text>
...
export default RastrilloAireadorScreen;