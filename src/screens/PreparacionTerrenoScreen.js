...
const PreparacionTerrenoScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Obteniendo último registro de actividad para ID_actividad: 5 y ID_trabajador:', userId);
        const activity = await getLastActivity(5, userId);
  ...
  const handleStartActivity = async () => {
    ...
    try {
      ...
      const result = await startActivity(5, userId, location.ID_ubicacion, currentDate, currentTime);
