...
import { View, Text, StyleSheet, Alert, TouchableOpacity, ImageBackground, Image, Platform } from 'react-native';
...
const CargaScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Obteniendo último registro de actividad para ID_actividad: 6 y ID_trabajador:', userId);
        const activity = await getLastActivity(6, userId);
  ...
  const handleStartActivity = async () => {
    ...
    try {
      ...
      const result = await startActivity(6, userId, location.ID_ubicacion, currentDate, currentTime);
