...
const EnfardadaScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Obteniendo último registro de actividad para ID_actividad: 4 y ID_trabajador:', userId);
        const activity = await getLastActivity(4, userId, 'En curso');
  ...
  useEffect(() => {
    ...
    } else if (lastActivity && lastActivity.Estado === 'Finalizada') {
      const startDateTime = new Date(`1970-01-01T${lastActivity.Hora_inicio}`);
      const endDateTime = new Date(`1970-01-01T${lastActivity.Hora_fin}`);
  ...
  const handleStartActivity = async () => {
    ...
    try {
      ...
      const result = await startActivity(4, userId, location.ID_ubicacion, currentDate, currentTime);
      console.log('Resultado de iniciar actividad:', result);

      Alert.alert('Éxito', 'Actividad iniciada correctamente.');

      console.log('Obteniendo última actividad después de iniciar para ID_actividad: 4 y ID_trabajador:', userId);
      const activity = await getLastActivity(4, userId, 'En curso');
    ...
    return (
      ...
      >
        ...
        <View style={styles.container}>
          ...
          <Text style={styles.subtitle}>ENFARDADA</Text>
    ...
    >
      ...
      <View style={styles.container}>
        ...
        <View style={styles.activityContainer}>
          ...
          <Text style={styles.activityValue}>
            {lastActivity && (lastActivity.Estado === 'En curso' || lastActivity.Estado === 'Finalizada')
              ? duration
              : 'No disponible'}
          </Text>

          {lastActivity && lastActivity.Estado === 'Finalizada' && (
            <>
              <Text style={styles.activityLabel}>Número de pacas de heno:</Text>
              <Text style={styles.activityValue}>
                {lastActivity.Pacas_heno || 'No disponible'}
              </Text>
            </>
          )}
...
export default EnfardadaScreen;